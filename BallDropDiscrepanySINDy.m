...
%Xi = (Theta_norm' * Theta_norm)^-1 * Theta_norm' * x_der(:, 2);
...
%% Discrepancy with Neural Network

net = feedforwardnet(10);
net = train(net, a_wDiscrepancy(:), (a_wDiscrepancy(:) - a_ideal(:)));

correctionNN = net(a_wDiscrepancy(:));

a_DiscrepancyNN = a_ideal(:) + correctionNN;
v_DiscrepancyNN = 0;
s_DiscrepancyNN = 0;
for i = 1 : length(t) - 1
    v_DiscrepancyNN(i+1) = v_DiscrepancyNN(i) + a_DiscrepancyNN(i) * dt;
    s_DiscrepancyNN(i+1) = s_DiscrepancyNN(i) + v_DiscrepancyNN(i) * dt;
end
%% Plot
clf;
subplot(3,1,1);
...
grid on;
grid minor;
plot(t, a_DiscrepancySINDy,'b--', t, a_DiscrepancyNN,'g-', t, a_wDiscrepancy,'r--', t, a_ideal, 'k-');
legend("Discrepancy SINDy", "Discrepancy NN", "Meas", "Ideal");
subplot(3,1,2);
title("Velocity [m/s]")
hold on;
grid on;
grid minor;
plot(t, v_DiscrepancySINDy,'b-', t, v_DiscrepancyNN,'g-', t, v_wDiscrepancy,'r--', t, v_ideal, 'k-');
legend("Discrepancy SINDy", "Discrepancy NN", "Meas", "Ideal");
subplot(3,1,3);
title("Distance [m]")
hold on;
grid on;
grid minor;
plot(t, s_DiscrepancySINDy,'b-', t, s_DiscrepancyNN,'g-', t, s_wDiscrepancy,'r--', t, s_ideal, 'k-');
legend("Discrepancy SINDy", "Discrepancy NN", "Meas", "Ideal");
