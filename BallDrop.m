...
for i = 1 : length(a_ideal)
    Theta(i,1:5) = [1, v_meas(i), s_meas(i), v_meas(i)*s_meas(i), v_meas(i)^2, ...
...
RMS_a = rms(x_der(:, 2));
x_der_norm(:, 2) = x_der(:, 2) / RMS_a;

%Xi= inv(Theta'*Theta) * Theta' * x_der(:, 2);
Xi= inv(Theta_norm'*Theta_norm) * Theta_norm' * x_der_norm(:, 2);
...
for k = 1:n_iter
    ...
    Xi_red = inv(Theta_new' * Theta_new) * Theta_new' * x_der_norm(:, 2);
...
a_IdealSINDy = (Theta_norm * Xi) .* RMS_a;
v_IdealSINDy = 0;
s_IdealSINDy = 0;
for i = 1 : length(t) - 1
...
% domináns függvények kiválasztása, jó-e az illesztés (normálttal)