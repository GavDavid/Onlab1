...
Theta_functions = {'1', 'v', 's', 'v*s', 'v^2', 's^2', 's^2*v', 's*v^2', 's^3', 'v^3'};
...
% Tikhonov regularizált least squares
...
Theta_reduced = Theta_norm;
Theta_reduced(:,remove_idx) = [];

Theta_functions_reduced = Theta_functions;
...
active_features(strcmp(Theta_functions,'v^3')) = false;
active_features(strcmp(Theta_functions,'s^3')) = false;
active_features(strcmp(Theta_functions,'v*s')) = false;
active_features(strcmp(Theta_functions,'s*v^2')) = false;
active_features(strcmp(Theta_functions,'s^2')) = false;
active_features(strcmp(Theta_functions,'s^2*v')) = false;
active_features(strcmp(Theta_functions,'v^2')) = false;

Theta_reduced = Theta_norm(:,active_features);

Theta_functions_reduced = Theta_functions(active_features);
...
Xi_reduced = V * diag(s_filt) * U' * x_der(:,2);




Xi = zeros(length(Theta_functions),1);

Xi(active_features) = Xi_reduced;
...
for i = 1:n_iter
    ...
    Xi_red = V * diag(s_filt) * U' * x_der(:,2);
...
v_rec(1) = v_meas(1);
for i = 1:(length(t) - 1)
    Theta_rec(i,1:10) = [1, v_rec(i), s_rec(i),...
    v_rec(i)*s_rec(i), v_rec(i)^2,...
    s_rec(i)^2,  s_rec(i)^2*v_rec(i), s_rec(i)*v_rec(i)^2,...
    s_rec(i)^3, v_rec(i)^3];
